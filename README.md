...
### v1.7.0
- ✨ 新增自动断点续传：处理期间每 60 秒自动保存，浏览器崩溃/刷新后可恢复
- ✨ 新增 API 智能重试：覆盖 429/500/502/503 等瞬态错误，酒馆 API 同样支持
- ✨ 新增并行批次间世界书摘要同步，分批模式下减少跨批次重复条目
- ✨ 新增处理进度 ETA 预估，基于滑动平均实时显示剩余时间
- ✨ 新增批量操作前自动快照（替换/整理/合并前自动保存历史）
- ✨ 新增处理完成后自动去重建议，提示疑似重复条目
- ✨ 新增 Prompt 模板可视化编辑，支持分标签页编辑世界书/剧情/文风模板
- ✨ 新增增量导出变更条目，仅导出上次导出后的新增/修改
- ✨ 新增响应过滤标签功能，支持黑名单移除 thinking 等标签内容
- 🔒 修复多处 XSS 注入漏洞，统一使用 escapeHtml 转义
- 🐛 修复长篇小说处理必定失败的问题（用紧凑摘要替代完整世界书注入）
- 🐛 修复数据完整性问题（DB 竞态、事务完整性、合并去重）
- 🐛 修复 ReDoS 防护、静默错误、文件限制、并发保护等中等问题
- 🐛 修复四个高优先级功能缺陷
- 📝 帮助弹窗和提示信息全面适配新功能
- 📝 重写 README 和 txtToWorldbook 模块文档

### v1.6.0
- ⚙️ TXT 转世界书模块完成模块化重构，拆分为 core/infra/services/ui/app 五层架构

### v1.5.0
- ⚙️ TXT 转世界书模块大修，开始模块化拆分
